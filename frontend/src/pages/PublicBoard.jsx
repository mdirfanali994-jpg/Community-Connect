...
const PublicBoard = () => {
  ...
  useEffect(() => {
    const fetchComplaints = async () => {
      try {
        ...
        const communityId =
          user?.communityId || user?.communityID || user?.community_id || null;

        // Strict tenant isolation: PublicBoard must never fetch global data.
        // If there is no authenticated society identity, show empty state.
        if (!communityId) {
          setComplaints([]);
          return;
        }
        ...
        const res = await axios.get(
          `${API_BASE_URL}/complaints?role=user&userId=` + encodeURIComponent(user?.id || ''),
  ...
  if (loading)
    return <div className="text-center py-20 text-gray-500 animate-pulse">Synchronizing Data...</div>;
